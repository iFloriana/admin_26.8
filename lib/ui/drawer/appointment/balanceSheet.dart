...
import '../../../wiget/custome_snackbar.dart'; // Assuming this is where CustomSnackbar is defined
import 'package:flutter_template/utils/colors.dart'; // Assuming this is where primaryColor is defined
...
class _DailySummaryScreenState extends State<DailySummaryScreen> {
  ...
  Future<void> fetchBranches() async {
    ...
    final url = '${Apis.baseUrl}/branches/names?salon_id=${getdata?.salonId}';
    try {
      final response = await dioClient.dio.get(url);
      if (response.statusCode == 200) {
        final json = response.data;
        if (json['message'] == 'Branch names and IDs fetched successfully') {
  ...
  Future<void> fetchData() async {
    ...
    String url =
        '${Apis.baseUrl}/expenses/daily-summary?start_date=$formattedStart&end_date=$formattedEnd&salon_id=${getdata?.salonId}';
        ...
        if (json['success'] == true) {
          ...
          data.forEach((dateKey, list) {
            for (var item in list as List<dynamic>) {
              ...
              entry['display_date'] = dateKey; // Use the date key for display
          ...
          PopupMenuButton<String?>(
            ...
            itemBuilder: (BuildContext context) {
              return [
                ...
                ...branches.map((branch) => PopupMenuItem<String?>(
