...
class ManagerStaffProfileScreen extends StatelessWidget {
  final ManagerStaffItem staff;
  ManagerStaffProfileScreen({super.key, required this.staff});
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: [
            ManagerStaffDetailsTab(staff: staff),
            ManagerAttendanceTab(staff: staff),
            ManagerPerformanceTab(staff: staff),
          ],
        ),
      ),
    );
...
class ManagerStaffDetailsTab extends StatelessWidget {
  final ManagerStaffItem staff;
  const ManagerStaffDetailsTab({super.key, required this.staff});
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildDetailRow('Branch', staff.branch?.name ?? 'N/A'),
          _buildDetailRow(
            'Services',
            staff.services.map((s) => s.name ?? 'N/A').join(', '),
          ),
          _buildDetailRow('Status', staff.status == 1 ? 'Active' : 'Inactive'),
          _buildDetailRow(
            'Show in Calendar',
            staff.showInCalendar == true ? 'Yes' : 'No',
          ),
        ],
      ),
    );
...
class ManagerAttendanceController extends GetxController {
  ...
  ManagerAttendanceController(this.staff);
...
class ManagerAttendanceTab extends StatelessWidget {
  final ManagerStaffItem staff;
  const ManagerAttendanceTab({super.key, required this.staff});

  @override
  Widget build(BuildContext context) {
    Get.put(ManagerAttendanceController(staff));
    return Obx(() {
      final controller = Get.find<ManagerAttendanceController>();
      return Stack(
        children: [
...
class ManagerPerformanceController extends GetxController {
  final Rx<Map<String, dynamic>?> performanceData =
      Rx<Map<String, dynamic>?>(null);
  final RxBool isLoading = true.obs;
  final ManagerStaffItem staff;

  ManagerPerformanceController(this.staff);
  ...
  Future<void> fetchPerformanceData() async {
    var getUser = await prefs.getManagerUser();
    try {
      final response = await dioClient.dio.get(
        '${Apis.baseUrl}/staffs/performance/${staff.id}',
        queryParameters: {'salon_id': getUser?.manager?.salonId},
      );
...
class ManagerPerformanceTab extends StatelessWidget {
  final ManagerStaffItem staff;
  const ManagerPerformanceTab({super.key, required this.staff});

  @override
  Widget build(BuildContext context) {
    final controller = Get.put(ManagerPerformanceController(staff));
